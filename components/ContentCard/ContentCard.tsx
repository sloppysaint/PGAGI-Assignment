    ...
    <div className="group relative bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden h-full flex flex-col">
      {hasImage && (
        <div className="relative overflow-hidden bg-gray-100 dark:bg-gray-700">
          ...
          <div className="absolute top-2 xs:top-3 left-2 xs:left-3">
            <span className={`inline-flex items-center gap-1 px-1.5 xs:px-2 py-0.5 xs:py-1 rounded-full text-xs xs:text-xs font-medium backdrop-blur-sm ${
              isMovie 
              ...
              {isMovie ? (
                <>
                  ...
                  <span className="hidden xs:inline">Movie</span>
      ...
      <div className="p-3 xs:p-4 flex flex-col flex-1">
        <div className="flex items-start gap-2 xs:gap-3 mb-2 xs:mb-3">
          <h2 className="font-bold text-sm xs:text-base sm:text-lg text-gray-900 dark:text-white leading-tight flex-1 line-clamp-2">
            {article.title || article.name}
          </h2>
          <button
            className="flex-shrink-0 relative p-1.5 xs:p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-all duration-200 group/fav touch-manipulation"
          ...
          >
            ...
            <div className="relative">
              {isFavorited ? (
                <svg className="w-4 h-4 xs:w-5 xs:h-5 text-red-500 animate-pulse" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clipRule="evenodd"/>
                </svg>
              ) : (
                <svg className="w-4 h-4 xs:w-5 xs:h-5 text-gray-400 group-hover/fav:text-red-400 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <p className="text-gray-600 dark:text-gray-300 text-xs xs:text-sm leading-relaxed mb-3 xs:mb-4 flex-1 line-clamp-2 xs:line-clamp-3">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-700">
          {article.url && (
            <a
              ...
              className="inline-flex items-center gap-1 xs:gap-2 text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-medium text-xs xs:text-sm transition-colors duration-200 group/link touch-manipulation"
            >
              <span className="hidden xs:inline">Read Article</span>
              <span className="xs:hidden">Read</span>
              <svg className="w-3 h-3 xs:w-4 xs:h-4 transition-transform duration-200 group-hover/link:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {article.id && !article.url && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1 xs:gap-2 text-purple-600 dark:text-purple-400 hover:text-purple-800 dark:hover:text-purple-300 font-medium text-xs xs:text-sm transition-colors duration-200 group/link touch-manipulation"
            >
          ...
          {article.vote_average && (
            <div className="flex items-center gap-1 text-yellow-600 dark:text-yellow-400">
              <svg className="w-3 h-3 xs:w-4 xs:h-4 fill-current" viewBox="0 0 20 20">
